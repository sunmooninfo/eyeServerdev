...
微信平台支付成功以后，会把支付结果推送到`common.notify-url`所指定的地址。
因此，开发者必须确定`common.notify-url`所指向的访问链接是可以成功访问，同时
能够返回正常的响应结果。
...
eye-db模块的application-db.yaml资源文件中reasonable是true
...
这里主要是指eye-db、eye-core和eye-all模块三个模块的相关问题。
...
```
org.apache.ibatis.binding.BindingException: Invalid bound statement (not found): org.linlinjava.eye.db.dao.XXXX
```
...
The error may exist in org/linlinjava/eye/db/dao/LitemallSystemMapper.xml
The error may involve org.linlinjava.eye.db.dao.LitemallSystemMapper.selectByExample-Inline
The error occurred while setting parameters
SQL: select id, key_name, key_value, add_time, update_time, deleted from eye_system
...
开发者直接（或使用Navicat）运行eye_schema.sql时运行失败。
...
首先，请开发者请直接打开eye_schema.sql文件，可以看到
```
drop database if exists eye;
drop user if exists 'eye'@'localhost';
create database eye default character set utf8mb4 collate utf8mb4_unicode_ci;
use eye;
create user 'eye'@'localhost' identified by 'eye123456';
grant all privileges on eye.* to 'eye'@'localhost';
flush privileges;
```
...
因此，如果开发者运行eye_schema.sql失败，开发者可以自行使用
相关SQL命令或者使用SQL工具创建数据库、用户和分配权限工作。

此外实际上，**开发者也不应该在部署或者上线阶段运行eye_schema.sql**
...
应该是eye-admin模块和eye-vue模块的node_modules文件夹导致的。
node_modules是eye-admin和eye-vue模块所依赖的项目库，可能有近200M的文件。
而IDEA如果没有设置，则可能尝试对该文件夹进行解析索引，从而导致卡断。

解决方案：

1. 先关闭IDEA，然后删除eye-admin和eye-vue模块内的node_modules文件夹；
2. 然后分别创建空的node_modules文件夹；
3. 重新打开IDEA，分别设置eye-admin模块和eye-vue模块的node_modules文件夹Excluded状态。
