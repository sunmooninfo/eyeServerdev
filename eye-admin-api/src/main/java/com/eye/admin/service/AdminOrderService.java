...
public class AdminOrderService {
    ...
    public Object list(Integer userId, String orderSn, LocalDateTime start, LocalDateTime end, List<Short> orderStatusArray,
        ...
        List<EyeOrder> orderList = orderService.querySelective(userId, orderSn, start, end, orderStatusArray, page, limit,
                sort, order);
        return ResponseUtil.okList(orderList);
    }

    public Object detail(Integer id) {
        EyeOrder order = orderService.findById(id);
        List<EyeOrderGoods> orderGoods = orderGoodsService.queryByOid(id);
     ...
     * 2. 管理员登录Eye管理后台点击退款操作进行订单状态修改和商品库存回库
     *
    ...
    public Object ship(String body) {
        ...
        notifyService.notifySmsTemplate(order.getMobile(), SmsNotifyType.SHIP, new String[]{shipChannel, shipSn});
    ...
    public Object reply(String body) {
        ...
        // 目前只支持回复一次
        EyeComment comment = commentService.findById(commentId);
        if(comment == null){
            return ResponseUtil.badArgument();
        }
